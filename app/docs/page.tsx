...
function DocH1({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <h1 id={id} className="scroll-mt-20 font-mono text-3xl font-bold text-zinc-100 mb-3">
      {children}
    </h1>
  );
}
function DocH2({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <h2 id={id} className="scroll-mt-20 font-mono text-xl font-semibold text-zinc-200 mt-12 mb-4 pt-2 border-t border-zinc-800/60">
      {children}
    </h2>
  );
}
...
function ApiRow({ method, path, desc }: { method: string; path: string; desc: string }) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-lg border border-zinc-800 bg-zinc-900/30 px-4 py-3 mb-2">
      <span className={cn("shrink-0 rounded border px-1.5 py-0.5 font-mono text-[10px] font-bold", colors[method] ?? "text-zinc-400")}>
        {method}
      </span>
      <code className="font-mono text-sm text-zinc-300 flex-1">{path}</code>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-14 border-b border-zinc-800/70 bg-zinc-950/90 backdrop-blur-md">
        <div className="flex h-full items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/"
              className="flex items-center gap-1.5 rounded-md border border-zinc-800 bg-zinc-900/60 px-3 py-1.5 font-mono text-xs text-zinc-400 transition-colors hover:border-zinc-700 hover:text-zinc-200"
            >
          ...
          <Link
            href="/app/dashboard"
            className="flex items-center gap-1.5 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-1.5 font-mono text-xs text-zinc-300 transition-colors hover:border-zinc-600 hover:bg-zinc-800"
          >
            Open Dashboard
      ...
      <div className="flex pt-14">
        {/* ── Left sidebar ── */}
        <aside className="fixed top-14 left-0 h-[calc(100vh-3.5rem)] w-60 shrink-0 overflow-y-auto border-r border-zinc-800/70 bg-zinc-950 py-6">
        ...
        {/* ── Main content ── */}
        <main className="ml-60 mr-52 min-h-screen flex-1 px-10 py-12 max-w-3xl">
...
{`Browser → Next.js API route → Ollama (127.0.0.1:11434)
         ...
         (no CORS issues)`}
          ...
          <DocP>
            Check whether the Ollama runtime is reachable from the DagOS server.
          </DocP>
      ...
      // append token to message...
          ...
          <div className="mt-16 border-t border-zinc-800/50 pt-8 pb-16">
            <div className="flex items-center justify-between">
              <p className="font-mono text-xs text-zinc-700">DagOS v0.1.0-alpha · MIT License</p>
        ...
        {/* ── Right TOC ── */}
        <aside className="fixed top-14 right-0 h-[calc(100vh-3.5rem)] w-52 shrink-0 overflow-y-auto py-8 px-5 border-l border-zinc-800/70 bg-zinc-950">
